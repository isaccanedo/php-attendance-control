...
function confirma(){
        if(confirm('Deseja realmente atender a chamada?'))
                return true;
        else
                return false;
}
...
if (($begin > 0) and ($begin <= 10)) {
   $anteriores = '<a href="ler_abertos.php?begin=0">Anteriores</a>';
} elseif (($begin > 0) and ($begin > 10)) {
   $anteriores = '<a href="ler_abertos.php?begin=' . ($begin-10) . '">Anteriores</a>';
} else {
   $anteriores = 'Anteriores';
}
...
if (($begin < $total) and (($begin+10) >= $total)) {
   ...
   $proximos = '<a href="ler_abertos.php?begin=' . ($begin+10) . '">Próximos</a>';
}
}
?>
<tr>
<td height="490">&nbsp;
    ...
    <table width="526" border="0" align="center" cellpadding="2" cellspacing="0">
        ...
        <tr>
          <td height="19" class="bodyText"><p align="center" class="style1">TOTAL CHAMADOS EM ABERTO: <span class="style3"><font color="#00FF40"><?php echo $total; ?></font></span></p>            </td>
        </tr>
        <tr>
...
$query = 'SELECT DISTINCT * FROM registros WHERE estado = "ABERTO" ORDER BY `data_abertura` ASC LIMIT '.$begin.' , 10';
$query = mysql_query($query);
   ...
   ?></p>
    <table width="526" border="0" align="center" cellpadding="2" cellspacing="0">
        ...
        <tr valign="top">
          ...
          <form id="atender" name="atender" method="post" action="update_atender.php">
            <label>
  <input name="id" type="hidden" id="id" value="<?php echo $linha['id']; ?>" />
            </label>
  <label>
  <input name="Submit" type="submit" class="botoes" value="ATENDER" onClick="return confirma()" />
  </label>
  &nbsp;</form></td>
      </tr>
        <tr align="right">
